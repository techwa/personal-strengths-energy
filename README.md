...
**方式一（推荐）**：下载本仓库的 [`personal-strengths-energy.skill`](personal-strengths-energy.skill) → 打开 Claude 设置 → Skills（能力）→ 上传该文件。

**方式二**：下载整个 `personal-strengths-energy/` 文件夹 → 压缩为 zip → 将后缀改为 `.skill` → 按上述方式上传。

**方式三（其他平台）**：本 Skill 只依赖 Markdown 指令。在不支持 Agent Skills 的平台（DeepSeek、豆包、Kimi 等），可将 `personal-strengths-energy/` 下的文件内容合并后作为对话开场的指令使用；长期记忆读取为可选能力，不支持时会自动降级为基于当前对话与你上传的材料。

安装后直接说："分析我的优势和能量来源" / "生成我的优势报告" / "什么给我充电"。
